...
class RuntimeConfigTests(unittest.TestCase):
    ...
    def test_production_rejects_missing_frontend_url(self) -> None:
        env = self.production_env()
        env.pop("FRONTEND_URL")

        with self.assertRaisesRegex(ValidationError, "FRONTEND_URL is required in production"):
            self.load_settings(env)

    def test_production_rejects_wildcard_cors(self) -> None:
        with self.assertRaisesRegex(ValidationError, "CORS_ORIGINS or FRONTEND_URL"):
